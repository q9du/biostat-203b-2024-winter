...
```
```{r}
first10_sorted <- labevents_arrow %>%
  slice_head(n = 10) %>%
  arrange(subject_id, charttime, itemid)
first10_sorted
```
The ingest+select+filter process takes 29.081 seconds.
...
```
```{r}
labevents_parquet <- open_dataset("labevents.parquet")
```
```{r}
...
first10_sorted_labevents_parquet <- labevents_parquet_result %>%
  slice_head(n = 10) %>%
  arrange(subject_id, charttime, itemid)
first10_sorted_labevents_parquet
```
The Parquet file `labevents.parquet` is 96 bytes in size.
The ingest+select+filter process of the Parquet file `labevents.parquet` takes 0.134 seconds.
